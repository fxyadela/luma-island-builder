...
8. 默认接入 `templates/default-luma-island.css` 和拖动模板，构建并验证一个可运行的桌面应用第一版；只打开浏览器页面不算完成。
...
8. Copy `templates/default-luma-island.css` and the drag helper by default, then build and verify a runnable desktop first version; a browser page alone is not complete.
