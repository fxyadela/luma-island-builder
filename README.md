...
`luma-island-builder` 是一个 Codex Skill，用来引导用户通过对话做出一个可运行的跨平台桌面工作岛。它默认做桌面应用，不是网页、localhost 预览或静态 HTML。
...
这个 Skill 会把“我想做一个桌面 Island”这种模糊想法，推进成一个能安装、能再次打开的第一版桌面应用。
...
- 默认按跨平台 Electron 桌面应用实现；Vite 只能作为渲染层，不能把结果停在网页
- 新项目默认使用内置 Electron 脚手架，直接生成桌面壳、预加载文件、渲染层、默认 CSS、拖动模板和打包脚本
- 默认交付成桌面应用：Mac 放进“应用程序”，Windows 有安装包、开始菜单入口或桌面快捷方式
- 默认复制当前桌面端同款 CSS 模板和头像资产，先给出透明窗口、液态 dock、收起态、头像球和面板外壳
...
8. 默认接入 `templates/default-luma-island.css`、`assets/prototypes/` 和拖动模板，构建并验证一个可运行的桌面应用第一版；只打开浏览器页面不算完成。
...
`luma-island-builder` is a Codex Skill for guiding users through building a small, runnable cross-platform desktop work island. By default it builds a desktop app, not a web page, localhost preview, or static HTML file.
...
This Skill turns a vague idea like "I want a desktop Island" into an installable desktop first version the user can open again.
...
- implement or guide a small cross-platform Electron desktop app; Vite may only be the renderer, not the final user-facing result
- use the bundled Electron scaffold for new projects, so the result starts with a desktop shell, preload file, renderer, default CSS, drag helper, and packaging scripts
- deliver a real desktop app: macOS Applications, Windows installer, Start menu entry, or desktop shortcut
- copy the current desktop shell CSS and avatar assets by default, so the shell starts with the same transparent window, liquid dock, collapsed entry, dock orb, and panel style
...
8. Copy `templates/default-luma-island.css`, `assets/prototypes/`, and the drag helper by default, then build and verify a runnable desktop first version; a browser page alone is not complete.
